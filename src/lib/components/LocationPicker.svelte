...
async function initMap() {
 ...
 function clearLocation() {
   ...
   dispatch('location-selected', null);
 }
</script>

<div class="location-picker">
 <div class="location-controls">
   <button type="button" on:click={getCurrentLocation} disabled={isGettingLocation}>
     {isGettingLocation ? '📍 Getting location...' : '📍 Use Current Location'}
   </button>

   {#if coordinates}
     <button type="button" on:click={clearLocation} class="secondary">Clear Location</button>
...
<style>
 ...
 .coordinates {
   ...
   padding: 0.25rem 0.5rem; background: #f8f9fa; border-radius: 4px;
 }
 .map-container {
   width: 100%; height: 320px; border: 2px solid #e9ecef; border-radius: 8px;
   overflow: hidden; background: #f0f0f0;
 }
 .error { color: #dc3545; font-size: 0.9rem; margin-bottom: 0.5rem; padding: 0.5rem; background: #f8d7da; border-radius: 4px; }
 button { cursor: pointer; }

 /* Fix Leaflet zoom controls alignment */
